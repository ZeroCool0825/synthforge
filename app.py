...
if "columns" not in st.session_state:
    st.session_state.columns = [
        ...
        {"name": "purchased", "type": "Boolean", "description": "Whether the customer completed an online purchase (True/False)"},
    ]
...
Now generate {num_rows} diverse, realistic rows following the same JSON structure. Return a JSON array with exactly {num_rows} objects."""
        ...
        # ── Batch settings ──────────────────────────────────────────
        BATCH_SIZE = 25  # rows per API call — stays well within token limits
        batches = []
